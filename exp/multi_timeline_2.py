...
def categorical_collate(batches, timeenc, freq):
    ...
    # valid_seq_x_id = torch.stack([torch.tensor(seq[0]) if isinstance(seq[0], np.ndarray) else seq[0] for seq in valid_ids])
    # valid_seq_y_id = torch.stack([torch.tensor(seq[1]) if isinstance(seq[1], np.ndarray) else seq[1] for seq in valid_ids])
